...
def sc():
    video = open_video('output/yt_dlp/test.mp4')

    # scenedetect 초기화
    scene_manager = SceneManager()
    # scene_manager.add_detector(scenedetect.detectors.ContentDetector())
    scene_manager.add_detector(ContentDetector(threshold=0.68))
    ...
    s.save_images(scene_list, video=video, output_dir='output/frame/')
